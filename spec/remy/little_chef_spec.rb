...
describe Remy::LittleChef do
  ...
  describe '#run' do
    it 'should work with a hash as its argument' do
      ...
      little_chef.run
      little_chef.configuration.ip_address.should == '50.57.162.227'
    ...
    it 'should not modify the global Remy config, but rather only the config which is for this particular Chef node' do
      ...
      little_chef.configuration.yml_files.should == ['../fixtures/foo.yml', '../fixtures/bar.yml', '../fixtures/little_chef.yml'].map {|f| File.join(File.dirname(__FILE__), f) }
